<x-app-layout>
        ...
        <div role="alert" id="cartel" class="rounded-xl border my-4 border-gray-100 bg-white p-4">
            <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                    ...
                    <p class="mt-1 text-sm text-gray-700">
                        {{ $message != 'El campo categoria ya ha sido registrado.' ? $message : 'La categoria que intenta ingresar ya se encuentra registrada.' }}
                    </p>
                </div>
                <button onclick="remove()" class="text-gray-500 transition hover:text-gray-600">
                    <span class="sr-only">Dismiss popup</span>
    ...
    <div>
            ...
            <table class=" w-full table-fixed   bg-white text-sm">
                ...
                <tbody class=" ">
                    @foreach ($categorias as $categoria)
                        <x-modal name="{{ $categoria->id }}">
                            <form action="{{ route('categories.update', $categoria) }}" method="POST" class="">
                                @method('PATCH')
                                @csrf
                                ...
                                <div class="p-6 flex flex-col justify-center">
                                    <div class="flex flex-col">
                                        <label for="categoria" class="block  mt-4 font-semibold text-gray-700">
                                            Nombre
                                        </label>
                                        <input type="text" required id="{{ $categoria->id }}" name="categoria"
                        ...
                        <tr class="odd:bg-purple-100 text-center ">
                            ...
                            <td class="whitespace-nowrap px-4 py-2 text-gray-700 ">
                                <div class="flex justify-center items-center">
                                    <button x-on:click.prevent="$dispatch('open-modal', '{{ $categoria->id }}')"
        ...
        <div class="m-2 p-2">{{ $categorias->links() }}</div>
    </div>
    <x-modal name="nueva">
        <form action="{{ route('categories.store') }}" method="POST" class="p-6">
            ...
            <div class="my-6 flex flex-col justify-center">
                <div class="flex flex-col">
                    ...
                    <input type="text" required name="categoria" placeholder="" value="{{ old('categoria') }}"
                        class="mt-1  rounded-md border-gray-200 shadow-sm sm:text-sm" />
                </div>
                <div class="mt-4">
                    <button class="ml-3 py-1 px-2  rounded class-name hover:bg-gray-200 bg-gray-100"
